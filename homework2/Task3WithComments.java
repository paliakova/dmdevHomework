...
public class Task3WithComments {
    ...
    private static double totalBrokerAccount(int startSalary, int sixMonthRaise,
        ...
        while (month <= monthOfWork - 1) {
            ...
            if (month % 6 == 1 && month > 6) {
                ...
                System.out.println(month + " - Б зарплата за этот месяц повысилась, индекс " + additionalSalaryIndex);
            }

            int currentSalary = startSalary + (additionalSalaryIndex * sixMonthRaise);
            System.out.println(month + " - Б зарплата в этом месяце " + currentSalary);

            double brokerCurrentMonth = (double) currentSalary / 10 * stockReturn;
            System.out.println("Б брокер в этом месяце - " + brokerCurrentMonth);
    ...
    private static double totalOwnAccount(int startSalary, int monthlyExpenses, int sixMonthRaise,
        ...
        while (month <= monthOfWork) {
            ...
            if (month % 6 == 1 && month > 6) {
                additionalSalaryIndex ++;
                System.out.println(month + " - зарплата за этот месяц повысилась, индекс " + additionalSalaryIndex);
            }

            int currentSalary = startSalary + (additionalSalaryIndex * sixMonthRaise);
            System.out.println(month + " - зарплата в этом месяце " + currentSalary);

            double savingsCurrentMonth = currentSalary - monthlyExpenses  // этот блок тут должен стоять?
                    - (0.1 * currentSalary);
            System.out.println("накопления в этом месяце - " + savingsCurrentMonth);
